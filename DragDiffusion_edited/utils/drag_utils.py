...
def point_tracking(F0, F1, handle_points, handle_points_init, args):
    with torch.no_grad():
        ...
        for i in range(len(handle_points)):
            ...
            # Cosine similarity between f0 and F1_neighbor
            cos_sim = torch.nn.functional.cosine_similarity(
                f0.unsqueeze(dim=-1).unsqueeze(dim=-1), F1_neighbor, dim=1
            )
            
            # Max similarity for handle points
            row, col = divmod(cos_sim.argmax().item(), cos_sim.shape[-1])
            handle_points[i][0] = r1 + row
            handle_points[i][1] = c1 + col
